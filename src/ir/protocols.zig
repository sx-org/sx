...
pub const ProtocolResolver = struct {
    ...
    pub fn isProtocolConstraint(self: ProtocolResolver, written: []const u8, source: ?[]const u8) bool {
        if (self.l.program_index.module_decls == null or self.l.program_index.flat_import_graph == null) {
            return self.l.program_index.contains(.protocol, written) or
                self.l.program_index.contains(.protocol_ast, written);
        }
    ...
    pub fn resolveProtocol(self: ProtocolResolver, written: []const u8, source: ?[]const u8) ?ResolvedProtocol {
        ...
        if (self.l.program_index.module_decls == null or self.l.program_index.flat_import_graph == null) {
            const pd = self.l.program_index.lookup(.protocol_ast, written) orelse return null;
    ...
    pub fn protocolMethodInfos(self: ProtocolResolver, proto_ty: ?TypeId, proto_name: []const u8) ?[]const ProtocolMethodInfo {
        const pd = if (proto_ty) |ty|
            ...
            self.l.program_index.lookup(.protocol, proto_name) orelse return null;
        return pd.methods;
    }
    ...
    pub fn registerProtocolDecl(self: ProtocolResolver, pd: *const ast.ProtocolDecl) void {
        ...
        if (pd.type_params.len > 0) {
            ...
            self.l.program_index.put(.protocol_ast, self.l.declId(.{ .protocol_decl = pd }, self.l.current_source_file), self.protocolIdentityName(pd), pd);
            return;
        }
        ...
        const proto_id = self.l.declId(.{ .protocol_decl = pd }, self.l.current_source_file);
        if (!self.l.program_index.contains(.protocol, identity_name))
            self.l.program_index.put(.protocol, proto_id, identity_name, protocol_info);
        if (!self.l.program_index.contains(.protocol_ast, identity_name))
            self.l.program_index.put(.protocol_ast, proto_id, identity_name, pd);
    ...
    /// `(i64) -> i64` as a reader sees it, for a diagnostic that quotes the head.
    fn spelledFnType(self: ProtocolResolver, params: []const TypeId, ret: TypeId) []const u8 {
        var out = std.ArrayList(u8).empty;
        out.append(self.l.alloc, '(') catch return "";
        for (params, 0..) |p, i| {
            if (i > 0) out.appendSlice(self.l.alloc, ", ") catch return "";
            out.appendSlice(self.l.alloc, self.l.formatTypeName(p)) catch return "";
        }
        out.appendSlice(self.l.alloc, ") -> ") catch return "";
        out.appendSlice(self.l.alloc, self.l.formatTypeName(ret)) catch return "";
        return out.items;
    }
    ...
    /// entry per conformer, which is the coherence rule.
    fn registerFnTypeImpl(
        self: ProtocolResolver,
        ib: *const ast.ImplBlock,
        fn_ty: *const Node,
        source: ?[]const u8,
    ) void {
        const target_node = ib.target_type_expr orelse return;
        const diags = self.l.diagnostics;
        // The conformer may be declared after the impl, so an impl whose target
        // does not resolve YET stays unregistered and a later pass retries it
        // against a settled type table. Every other exit is final.
        var retry = false;
        defer if (!retry) self.l.registered_protocol_impls.put(ib, {}) catch @panic("out of memory");

        const at = target_node.span;
        ...
        if (!self.l.typeNodeLeavesReady(target_node, source)) {
            retry = true;
            return;
        }
        const cty = self.l.resolveTypeInSource(source, target_node);
        if (cty == .unresolved) {
            retry = true;
            return;
        }
        if (!self.isNominalConformer(cty)) {
            if (diags) |d| {
                const id = d.addFmtId(.err, at, "'{s}' cannot carry a function-type impl", .{self.l.formatTypeName(cty)});
        ...
        if (self.l.callable_nominals.get(cty)) |first| {
            if (diags) |d| {
                const id = d.addFmtId(.err, first.span, "'{s}' already carries a function-type impl", .{self.l.formatTypeName(cty)});
                d.addNoteFmt(id, at, "also implemented here", .{});
            }
            return;
        }

        var call_fd: ?*const ast.FnDecl = null;
        for (ib.methods) |method_node| {
            if (method_node.data != .fn_decl) continue;
            const fd = &method_node.data.fn_decl;
            const qualified = std.fmt.allocPrint(self.l.alloc, "{s}.{s}", .{ ib.target_type, fd.name }) catch continue;
            if (!self.l.program_index.contains(.function, qualified)) {
                self.l.program_index.registerFunction(qualified, fd, self.l.current_source_file);
            }
        ...
        if (fd.params.len == 0 or (recv_ty != cty and recv_ty != ptr_cty)) {
            if (diags) |d| {
                ...
                d.addHelpFmt(id, fd.name_span, null, "write 'call :: (self: *{s}, …)'", .{self.l.formatTypeName(cty)});
            }
            return;
        }
        ...
        for (fte.param_types) |p| head_params.append(self.l.alloc, self.l.resolveTypeInSource(source, p)) catch return;
        const head_ret = if (fte.return_type) |rt| self.l.resolveTypeInSource(source, rt) else TypeId.void;
        const agrees = head_ret == ret and head_params.items.len == params.items.len and
            std.mem.eql(TypeId, head_params.items, params.items);
        if (!agrees) {
            if (diags) |d| {
                const id = d.addFmtId(.err, fd.name_span, "'call' is '{s}', and the impl head is '{s}'", .{
                    self.spelledFnType(params.items, ret), self.spelledFnType(head_params.items, head_ret),
                });
                d.addHelpFmt(id, fd.name_span, null, "the head is the signature the conformer is callable at, so 'call' carries it past its receiver", .{});
            }
            return;
        }

        self.l.callable_nominals.put(cty, .{
            .fd = fd,
            .qualified = std.fmt.allocPrint(self.l.alloc, "{s}.call", .{ib.target_type}) catch return,
            .params = params.toOwnedSlice(self.l.alloc) catch return,
            .ret = ret,
            .span = at,
        }) catch @panic("out of memory");
    ...
    pub fn registerImplBlock(self: ProtocolResolver, ib: *const ast.ImplBlock, decl: *const Node) void {
        if (self.l.registered_protocol_impls.contains(ib)) return;
        const source = decl.source_file orelse self.l.current_source_file;
        // A function-type head names no protocol: it makes the conformer
        // callable and answers to its own registry.
        if (ib.protocol_fn_type) |fn_ty| return self.registerFnTypeImpl(ib, fn_ty, source);
        ...
        if (ib.protocol_type_args.len > 0) {
            ...
            self.registerParamImpl(ib, decl, proto_name);
        ...
        for (ib.methods) |method_node| {
            if (method_node.data == .fn_decl) {
                ...
                if (!self.l.program_index.contains(.function, qualified)) {
                    self.l.program_index.registerFunction(qualified, method_fd, self.l.current_source_file);
                }
        ...
        // Synthesize default methods from protocol declaration
        {
            ...
            for (pd.methods) |method| {
                if (method.default_body != null and !impl_methods.contains(method.name)) {
                    ...
                    if (!self.l.program_index.contains(.function, qualified)) {
                        self.l.program_index.registerFunction(qualified, synth_fd, self.l.current_source_file);
                    }
        ...
        for (decls) |decl| {
            switch (decl.data) {
                .impl_block => {
                    ...
                    if (ib.protocol_fn_type != null) {
                        diags.addFmt(.err, decl.span, "a function-type impl cannot register: its conformer '{s}' does not resolve in this module", .{ib.target_type});
                    } else if (self.resolveProtocol(ib.protocol_name, source) == null) {
    ...
    pub fn matchGenericParamImpl(
        ...
        // its bindings by.
        const tmpl_decl = self.l.program_index.lookup(.struct_template, template) orelse return false;
    ...
    pub fn blanketMethod(
        ...
        const tmpl_decl = self.l.program_index.lookup(.struct_template, template) orelse return null;
        for (entries.items) |entry| {
            if (entry.arg_nodes.len != arg_tys.len) continue;
            if (!std.mem.eql(u8, entry.target_template, template)) continue;
            if (entry.target_arg_nodes.len != tmpl_decl.type_params.len) continue;
    ...
    pub fn registerParamImpl(self: ProtocolResolver, ib: *const ast.ImplBlock, decl: *const Node, proto_name: []const u8) void {
        const table = &self.l.module.types;
        const source = decl.source_file orelse self.l.current_source_file;
        const saved_source = self.l.current_source_file;
        ...
        const src_ty: TypeId = if (ib.target_type_expr) |te| blk: {
            ...
            if (typeExprHasGeneric(te))
                break :blk type_bridge.resolveAstType(te, table, &self.l.program_index);
            ...
            if (ib.target_type_params.len > 0)
                break :blk type_bridge.resolveAstType(&node, table, &self.l.program_index);
        ...
        // lazy in param_impl_map and are handled below).
        {
            ...
            if (!src_ty.isBuiltin() and si == .@"struct") {
                ...
                const is_generic_src = self.l.program_index.contains(.struct_template, src_name);
                if (!is_generic_src) {
                ...
                for (methods.items) |mfd| {
                    ...
                    if (!self.l.program_index.contains(.function, q)) {
                        self.l.program_index.registerFunction(q, mfd, self.l.current_source_file);
                    }
                    if (!is_generic_src) {
