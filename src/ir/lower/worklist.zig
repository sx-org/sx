...
//! and every `@contextExtend` it spells is a field the program Context may
...
const Item = struct {
    ...
    /// How many `@contextExtend` declarations its unexpanded branches could
...
pub const Worklist = struct {
    ...
    /// How many `@contextExtend` declarations unretired drivers could still
...
const Scan = struct {
    ...
    /// alias is. Impls and `@contextExtend` are program-global either way.
