...
pub fn tryLowerErrorSetEquality(self: *Lowering, bop: *const ast.BinaryOp) ?Ref {
    ...
    if (l.set) |lc| if (r.set) |rc| {
        if (!errorSetValueRetypeIsLegal(self, lc, rc) and !errorSetValueRetypeIsLegal(self, rc, lc)) {
            if (self.diagnostics) |diags| {
                diags.addFmt(.err, bop.lhs.span, "error channels '{s}' and '{s}' do not compare — one channel must hold the other's members", .{ self.formatSourceTypeName(lc), self.formatSourceTypeName(rc) });
            }
            return self.builder.constBool(false);
        }
    };
...
fn channelPhrase(self: *Lowering, channel: ?TypeId) []const u8 {
    ...
    return std.fmt.allocPrint(self.alloc, "the error channel '{s}'", .{self.formatSourceTypeName(c)}) catch unreachable;
}
...
pub fn emitMemberNotInChannel(self: *Lowering, chan: TypeId, prefix: []const u8, leaf: []const u8, span: ast.Span) void {
    const d = self.diagnostics orelse return;
    const id = d.addFmtId(.err, span, "the error channel '{s}' does not carry '{s}.{s}' — this arm can never match", .{ self.formatSourceTypeName(chan), prefix, leaf });
