//! Context extension — `@contextExtend` collection pass
//! (design/context-extension.md).
//!
//! Gathers every `@contextExtend` declaration across the compilation into
...
/// Program-index collection pass for `@contextExtend` (pass 0a family; runs
...
pub fn collectContextExtensions(self: *Lowering, decls: []const *const Node) void {
    ...
    for (ext) |*e| {
        ...
        diags.addFmtInFile(.err, e.module_path, e.span, "@contextExtend '{s}' has no default value — the default context must be constructible before `main` runs", .{e.name});
    }
...
/// fields come from `@contextExtend` declarations, in sort order). The
...
/// Attach the registered `@contextExtend` field list (name, declared type
...
/// Serialize the `@contextExtend` declaration named `fname`'s default into a
...
/// Is `fname` a collected `@contextExtend` declaration (valid or not)?
