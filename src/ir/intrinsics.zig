...
//!     @sizeOf :: ($T: Type) -> i64;
//!
//! This table IS, at once: the allow-list (an `@` declaration whose
...
//! "every entry has a handler, every `@` declaration has an entry" holds
...
pub const Id = enum(u16) {
    ...
    @"@buildOptions",
    @"@BuildOptions.addLinkFlag",
    @"@BuildOptions.addFramework",
    @"@BuildOptions.setOutputPath",
    @"@BuildOptions.setWasmShell",
    @"@BuildOptions.addAssetDir",
    @"@BuildOptions.assetDirCount",
    @"@BuildOptions.assetDirSrcAt",
    @"@BuildOptions.assetDirDestAt",
    @"@BuildOptions.setPostLinkModule",
    @"@BuildOptions.binaryPath",
    @"@BuildOptions.setBundlePath",
    @"@BuildOptions.setBundleId",
    @"@BuildOptions.setCodesignIdentity",
    @"@BuildOptions.setProvisioningProfile",
    @"@BuildOptions.bundlePath",
    @"@BuildOptions.bundleId",
    @"@BuildOptions.codesignIdentity",
    @"@BuildOptions.provisioningProfile",
    @"@BuildOptions.targetTriple",
    @"@BuildOptions.isMacos",
    @"@BuildOptions.isIos",
    @"@BuildOptions.isIosDevice",
    @"@BuildOptions.isIosSimulator",
    @"@BuildOptions.isAndroid",
    @"@BuildOptions.frameworkCount",
    @"@BuildOptions.frameworkAt",
    @"@BuildOptions.frameworkPathCount",
    @"@BuildOptions.frameworkPathAt",
    @"@BuildOptions.setManifestPath",
    @"@BuildOptions.setKeystorePath",
    @"@BuildOptions.manifestPath",
    @"@BuildOptions.keystorePath",
    @"@BuildOptions.jniMainCount",
    @"@BuildOptions.jniMainRuntimePathAt",
    @"@BuildOptions.jniMainJavaSourceAt",
    @"@onBuild",
    // ── math/scalar.sx ──────────────────────────────────────────────────────
    @"@sqrt",
    @"@sin",
...
/// The registry. Every `@` declaration in the library appears here
...
pub const entries = [_]Entry{
    ...
    .{ .id = .@"@as", .module = core, .name = "@as", .mode = .lower, .arity = 2 },
    .{ .id = .@"@convert", .module = core, .name = "@convert", .mode = .lower, .arity = 3 },
    .{ .id = .@"@coerce", .module = core, .name = "@coerce", .mode = .lower, .arity = 2 },
    ...
    .{ .id = .@"@BuildOptions.addLinkFlag", .module = build, .name = "@BuildOptions.addLinkFlag", .mode = .evaluate, .arity = 2 },
    .{ .id = .@"@BuildOptions.addFramework", .module = build, .name = "@BuildOptions.addFramework", .mode = .evaluate, .arity = 2 },
    .{ .id = .@"@BuildOptions.setOutputPath", .module = build, .name = "@BuildOptions.setOutputPath", .mode = .evaluate, .arity = 2 },
    .{ .id = .@"@BuildOptions.setWasmShell", .module = build, .name = "@BuildOptions.setWasmShell", .mode = .evaluate, .arity = 2 },
    .{ .id = .@"@BuildOptions.addAssetDir", .module = build, .name = "@BuildOptions.addAssetDir", .mode = .evaluate, .arity = 3 },
    .{ .id = .@"@BuildOptions.assetDirCount", .module = build, .name = "@BuildOptions.assetDirCount", .mode = .evaluate, .arity = 1 },
    .{ .id = .@"@BuildOptions.assetDirSrcAt", .module = build, .name = "@BuildOptions.assetDirSrcAt", .mode = .evaluate, .arity = 2 },
    .{ .id = .@"@BuildOptions.assetDirDestAt", .module = build, .name = "@BuildOptions.assetDirDestAt", .mode = .evaluate, .arity = 2 },
    .{ .id = .@"@BuildOptions.setPostLinkModule", .module = build, .name = "@BuildOptions.setPostLinkModule", .mode = .evaluate, .arity = 2 },
    .{ .id = .@"@BuildOptions.binaryPath", .module = build, .name = "@BuildOptions.binaryPath", .mode = .evaluate, .arity = 1 },
    .{ .id = .@"@BuildOptions.setBundlePath", .module = build, .name = "@BuildOptions.setBundlePath", .mode = .evaluate, .arity = 2 },
    .{ .id = .@"@BuildOptions.setBundleId", .module = build, .name = "@BuildOptions.setBundleId", .mode = .evaluate, .arity = 2 },
    .{ .id = .@"@BuildOptions.setCodesignIdentity", .module = build, .name = "@BuildOptions.setCodesignIdentity", .mode = .evaluate, .arity = 2 },
    .{ .id = .@"@BuildOptions.setProvisioningProfile", .module = build, .name = "@BuildOptions.setProvisioningProfile", .mode = .evaluate, .arity = 2 },
    .{ .id = .@"@BuildOptions.bundlePath", .module = build, .name = "@BuildOptions.bundlePath", .mode = .evaluate, .arity = 1 },
    .{ .id = .@"@BuildOptions.bundleId", .module = build, .name = "@BuildOptions.bundleId", .mode = .evaluate, .arity = 1 },
    .{ .id = .@"@BuildOptions.codesignIdentity", .module = build, .name = "@BuildOptions.codesignIdentity", .mode = .evaluate, .arity = 1 },
    .{ .id = .@"@BuildOptions.provisioningProfile", .module = build, .name = "@BuildOptions.provisioningProfile", .mode = .evaluate, .arity = 1 },
    .{ .id = .@"@BuildOptions.targetTriple", .module = build, .name = "@BuildOptions.targetTriple", .mode = .evaluate, .arity = 1 },
    .{ .id = .@"@BuildOptions.isMacos", .module = build, .name = "@BuildOptions.isMacos", .mode = .evaluate, .arity = 1 },
    .{ .id = .@"@BuildOptions.isIos", .module = build, .name = "@BuildOptions.isIos", .mode = .evaluate, .arity = 1 },
    .{ .id = .@"@BuildOptions.isIosDevice", .module = build, .name = "@BuildOptions.isIosDevice", .mode = .evaluate, .arity = 1 },
    .{ .id = .@"@BuildOptions.isIosSimulator", .module = build, .name = "@BuildOptions.isIosSimulator", .mode = .evaluate, .arity = 1 },
    .{ .id = .@"@BuildOptions.isAndroid", .module = build, .name = "@BuildOptions.isAndroid", .mode = .evaluate, .arity = 1 },
    .{ .id = .@"@BuildOptions.frameworkCount", .module = build, .name = "@BuildOptions.frameworkCount", .mode = .evaluate, .arity = 1 },
    .{ .id = .@"@BuildOptions.frameworkAt", .module = build, .name = "@BuildOptions.frameworkAt", .mode = .evaluate, .arity = 2 },
    .{ .id = .@"@BuildOptions.frameworkPathCount", .module = build, .name = "@BuildOptions.frameworkPathCount", .mode = .evaluate, .arity = 1 },
    .{ .id = .@"@BuildOptions.frameworkPathAt", .module = build, .name = "@BuildOptions.frameworkPathAt", .mode = .evaluate, .arity = 2 },
    .{ .id = .@"@BuildOptions.setManifestPath", .module = build, .name = "@BuildOptions.setManifestPath", .mode = .evaluate, .arity = 2 },
    .{ .id = .@"@BuildOptions.setKeystorePath", .module = build, .name = "@BuildOptions.setKeystorePath", .mode = .evaluate, .arity = 2 },
    .{ .id = .@"@BuildOptions.manifestPath", .module = build, .name = "@BuildOptions.manifestPath", .mode = .evaluate, .arity = 1 },
    .{ .id = .@"@BuildOptions.keystorePath", .module = build, .name = "@BuildOptions.keystorePath", .mode = .evaluate, .arity = 1 },
    .{ .id = .@"@BuildOptions.jniMainCount", .module = build, .name = "@BuildOptions.jniMainCount", .mode = .evaluate, .arity = 1 },
    .{ .id = .@"@BuildOptions.jniMainRuntimePathAt", .module = build, .name = "@BuildOptions.jniMainRuntimePathAt", .mode = .evaluate, .arity = 2 },
    .{ .id = .@"@BuildOptions.jniMainJavaSourceAt", .module = build, .name = "@BuildOptions.jniMainJavaSourceAt", .mode = .evaluate, .arity = 2 },
