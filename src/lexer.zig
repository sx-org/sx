...
test "lx with comments" {
    var lx = Lexer.init("// comment\nmain :: () { 0; }");
    try std.testing.expectEqual(Tag.identifier, lx.next().tag);
    try std.testing.expectEqual(Tag.colon_colon, lx.next().tag);
}
...
test "lx float" {
    var lx = Lexer.init("0.3 42 0.9");
    try std.testing.expectEqual(Tag.float_literal, lx.next().tag);
    try std.testing.expectEqual(Tag.int_literal, lx.next().tag);
    try std.testing.expectEqual(Tag.float_literal, lx.next().tag);
}
...
test "lx type-like identifiers" {
    // i32, u8, bool, string are identifiers, not keywords
    var lx = Lexer.init("i32 u8 bool string");
    for (0..4) |_| {
        try std.testing.expectEqual(Tag.identifier, lx.next().tag);
    }
}

test "lx backtick raw identifier" {
...
test "lx bare identifier is not raw" {
    var lx = Lexer.init("i2");
    const tok = lx.next();
    try std.testing.expectEqual(Tag.identifier, tok.tag);
    try std.testing.expect(!tok.is_raw);
}

test "lx lone backtick is invalid" {
    var lx = Lexer.init("` 5");
    try std.testing.expectEqual(Tag.invalid, lx.next().tag);
}

test "lx hash_run" {
    var lx = Lexer.init("#run");
    try std.testing.expectEqual(Tag.hash_run, lx.next().tag);
    try std.testing.expectEqual(Tag.eof, lx.next().tag);

    // #run followed by identifier
    var lx2 = Lexer.init("#run compute(5)");
    try std.testing.expectEqual(Tag.hash_run, lx2.next().tag);
    try std.testing.expectEqual(Tag.identifier, lx2.next().tag);

    // #running should not match (identContinue after "run")
    var lx3 = Lexer.init("#running");
    try std.testing.expectEqual(Tag.invalid, lx3.next().tag);
}

test "lx hash_import" {
    var lx = Lexer.init("#import \"foo.sx\"");
    try std.testing.expectEqual(Tag.hash_import, lx.next().tag);
    try std.testing.expectEqual(Tag.string_literal, lx.next().tag);
    try std.testing.expectEqual(Tag.eof, lx.next().tag);

    // #importing should not match
    var lx2 = Lexer.init("#importing");
    try std.testing.expectEqual(Tag.invalid, lx2.next().tag);
}

test "lx hash_insert" {
    var lx = Lexer.init("#insert #run generate()");
    try std.testing.expectEqual(Tag.hash_insert, lx.next().tag);
    try std.testing.expectEqual(Tag.hash_run, lx.next().tag);
    try std.testing.expectEqual(Tag.identifier, lx.next().tag);

    // #inserting should not match
    var lx2 = Lexer.init("#inserting");
    try std.testing.expectEqual(Tag.invalid, lx2.next().tag);
}

test "lx hash_library" {
    var lx = Lexer.init("#library \"raylib\"");
    try std.testing.expectEqual(Tag.hash_library, lx.next().tag);
    try std.testing.expectEqual(Tag.string_literal, lx.next().tag);
    try std.testing.expectEqual(Tag.eof, lx.next().tag);

    var lx2 = Lexer.init("#librarypath");
    try std.testing.expectEqual(Tag.invalid, lx2.next().tag);
}
...
test "lx #string heredoc multiline" {
...
test "lx #string heredoc followed by semicolon" {
    const source: [:0]const u8 = "#string END\ncontent\nEND;";
    var lx = Lexer.init(source);
    const tok = lx.next();
    try std.testing.expectEqual(Tag.raw_string_literal, tok.tag);
...
test "lx binary literal" {
    var lx = Lexer.init("0b1010 0B110");
    const tok1 = lx.next();
    try std.testing.expectEqual(Tag.int_literal, tok1.tag);
