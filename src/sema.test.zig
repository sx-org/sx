...
// while a BARE `i2` stays the builtin int. Sema's `resolveTypeNode` honours
...
//
// A COMPOUND raw type (`*`i2`, `?`i2`, `[N]`i2`, …) stores its inner name as a
...
// user type named `i2` as the builtin int, diverging from codegen. These pin
...
test "sema: field access through a raw `*`i2` pointer resolves the user field; bare `*i2` stays builtin" {
    ...
    // RAW: `p: *`i2` → field `x` on the user struct → i64. (Reclassifying the
    // pointee `i2` to the 2-bit int loses `.x` → unresolved.)
...
test "sema: unwrapping a raw `?`i2` optional resolves the user field; bare `?i2` stays builtin" {
    ...
    // (The optional child `i2` must not reclassify to the 2-bit int.)
    const y = symType(res, "y") orelse return error.MissingSymbol;
    try std.testing.expect(y == .signed);
    try std.testing.expectEqual(@as(u8, 64), y.signed);
...
test "sema: indexing a raw `[N]`i2` array resolves the user element; bare `[N]i2` stays builtin" {
    ...
    // RAW: `a: [4]`i2` → element is the user struct, never the 2-bit int.
    const y = symType(res, "y") orelse return error.MissingSymbol;
