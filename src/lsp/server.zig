...
const sx = struct {
    ...
    pub const token = @import("sxlex").token;
...
pub const Server = struct {
    ...
    fn handleCompletion(self: *Server, id: ?std.json.Value, params: std.json.Value) !void {
        ...
        const builtins = [_]struct { label: []const u8, detail: []const u8 }{
            .{ .label = "@typeOf", .detail = "(val: $T) -> Type" },
            ...
            .{ .label = "@field", .detail = "(v: any, idx: i64) -> any — part view" },
            .{ .label = "@inner", .detail = "(v: any) -> ?any — the optional's payload view" },
            .{ .label = "@unbox", .detail = "($T: Type, v: any) -> T — unchecked typed read" },
            .{ .label = "@tag", .detail = "(v: $T) -> $R — an enum value's tag" },
            .{ .label = "@as", .detail = "($T: Type, v: $S) -> T — the compiler's conversions" },
            .{ .label = "@This", .detail = "() -> Type — the enclosing struct, union, enum or member" },
            .{ .label = "@sizeOf", .detail = "(T | tp: Type) -> i64" },
            .{ .label = "@alignOf", .detail = "(T | tp: Type) -> i64" },
    ...
    fn handleSignatureHelp(self: *Server, id: ?std.json.Value, params: std.json.Value) !void {
        ...
        const builtin_sigs = [_]struct { name: []const u8, label: []const u8, params: []const []const u8 }{
            ...
            .{ .name = "@field", .label = "@field(v: any, idx: i64) -> any", .params = &.{ "v: any", "idx: i64" } },
            .{ .name = "@inner", .label = "@inner(v: any) -> ?any", .params = &.{"v: any"} },
            .{ .name = "@typeEq", .label = "@typeEq($A: Type, $B: Type) -> bool", .params = &.{ "$A: Type", "$B: Type" } },
            .{ .name = "@unbox", .label = "@unbox($T: Type, v: any) -> T", .params = &.{ "$T: Type", "v: any" } },
            .{ .name = "@tag", .label = "@tag(v: $T) -> $R", .params = &.{"v: $T"} },
            .{ .name = "@as", .label = "@as($T: Type, v: $S) -> T", .params = &.{ "$T: Type", "v: $S" } },
            .{ .name = "@This", .label = "@This() -> Type", .params = &.{} },
            .{ .name = "@sizeOf", .label = "@sizeOf($T: Type) -> i64", .params = &.{"$T: Type"} },
            .{ .name = "@alignOf", .label = "@alignOf($T: Type) -> i64", .params = &.{"$T: Type"} },
