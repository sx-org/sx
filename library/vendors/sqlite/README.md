...
On Linux, the module adds `-lm` to the link for FTS5 ranking.
...
build omits (sessions/snapshots, deprecated API). The loadable-extension
entry points are compiled but unbound, as are the FTS5 custom-tokenizer
and auxiliary-function callback APIs.

`SQLITE_ENABLE_FTS5` compiles in the built-in FTS5 virtual-table, query,
and ranking surface. It is available through SQL — `CREATE VIRTUAL TABLE
… USING fts5`, `MATCH`, `bm25()`, and the `rank` column.
