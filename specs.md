...
- `@typeInfo(T).struct.fields[i].type` / `@typeInfo(T).enum.fields[i].payload` — the reflected member's type, itself a type (`@typeInfo(pointeeType(@typeInfo(T).struct.fields[i].type))`). The projected variant must be the one `@typeInfo` reflects `T` into; a tagless variant's payload is `void`.
