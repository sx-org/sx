# Execution-model evolution (comptime JIT · async · concurrency · hot-reload)

> The design-of-record for sx's execution model across five interlocking
> threads. Cross-platform linking is covered separately in
> [bundled-zig-link-backend-design.md](bundled-zig-link-backend-design.md).
...
compiler." The **compiler** obligations this design carries, none of which the tree
has: **atomics lowering** (N1), **`declare` + `define` + `type_info` + `field_type`**
(comptime type metaprogramming), an emitting **`abi(.naked)`**, **per-fiber `context`
root** (+ per-fiber stack-limit), the **S1 persistent JIT spine**, **C1 thunk
synthesis**, **comptime-asm lifting** (C3), and the **S2 ORC C++ shim**. Async itself
is genuinely a library; the *enabling primitives* are a major codegen/runtime
investment. The pieces they build on — `inline asm`, `extern`/`export`, the
`!`/`try`/`catch`/`onfail`/`raise` error channel, value-level reflection, the
`sx run` ORC LLJIT, and the host-FFI trampolines — are in the tree.
...
| ID | Piece | What | Size |
|----|-------|------|------|
| **S1** | Persistent JIT executor | A long-lived ORC LLJIT + a host-triple `LLVMEmitter` + a compiled-fragment cache, plumbed into the interpreter. The LLJIT exists only for `sx run`'s `main` ([target.zig](../src/target.zig)); the emitter carries one target machine ([emit_llvm.zig](../src/ir/emit_llvm.zig)). | L |
| **S2** | ORC C++ shim | `MachOPlatform::Create` + redirectable/lazy-reexport symbols. The bare `LLVMOrcCreateLLJIT` cannot do thread-locals, C constructors, or symbol redefinition (a `_Thread_local` under it aborts), so it is required by any non-trivial JIT or symbol repoint. | M |
...
The bundler runs on the interpreter plus C1: it is I/O-bound, so native speed is
moot and C1 closes the only capability gap. JIT-ing the bundler subgraph is an
S1 optimization available if the bundler's *own* logic ever profiles hot.
...
| ID | Piece | State | Size |
|----|-------|-------|------|
| **N1** | **Atomics.** Atomic load/store/RMW (`add/sub/and/or/xor/swap` + `fetch_min`/`fetch_max`; no `nand`), `compare_exchange`/`_weak` (→ `?T`, **null = success**), and fences, with orderings (relaxed/acquire/release/acq_rel/seq_cst). LLVM provides all — an **emit** feature, not a runtime library. **Surface LOCKED = `Atomic($T)` wrapper + `Ordering` enum** (not `@atomic_*` — `@` is address-of in sx). | the compiler emits no LLVM `atomicrmw`/`cmpxchg`/`fence`, and `library/` declares no `Atomic`/`Ordering`; the "ordering" in `lower.zig` is *comparison* ordering (`< <= >=`), unrelated to memory ordering | M |
| **N2** | **OS threads + pthread Mutex/Cond + worker Pool** | [std/thread.sx](../library/modules/std/thread.sx) (`pthread_create`/`join`/`detach`, in-place `Mutex`/`Cond`, bounded `Pool`). NOTE: pthread mutex **blocks the OS thread** — it is *not* fiber-aware (it would park every fiber on that thread); fiber-aware sync is N3, built on N1. | — |
...
`push Context` stack. Ambient state that outlives a suspension point must be carried by the fiber, the
same rule as capturing an owning allocator, one level up.
...
synthesis.* Reflection only **reads** types (`field_count`/`field_name`/`type_of`);
`RaceResult(T)` must **construct** an anonymous `enum` from a tuple's
`(label, payload-type)` pairs. Supporting pieces: a `field_type($T, i) -> Type`
reflection accessor (value-level `field_value` + `type_of` exist; type-only field
projection does not) and `Future(T) → T` projection (falls out of generics). This
is the generic "derive a sum from a product" — useful beyond `race`.
...
reloads. Code/state separation — the owning-allocator discipline, one level up.
...
## 6. Cross-platform — from a macOS laptop

### 6.1 What the host already reaches

| Capability | Reach from a mac |
|---|---|
| `extern`/`export` C linkage | all targets |
| Bundled-`zig cc` cross-link backend | macOS, Linux(-musl/static), Windows(-gnu) |
| sx-side bundler (`.app`/`.apk`) | macOS, iOS sim/device, Android |
| JIT `sx run` (ORC LLJIT) | host |
| Target shorthands | `macos[-arm]`, `linux[-musl[-arm]]`, `windows[-gnu]`, `ios[-arm]`, `ios-sim[-arm/-x86]`, `android[-arm64/-x86_64]`, `wasm` |
...
# Linux (cross): static, zero-dep ELF
sx build app.sx --target linux-musl -o app      # scp anywhere, runs

# Windows (cross, MinGW path): PE32+
...
- **C1** → the iOS/Android **bundlers** (orchestrate ~a dozen host tools at
...
- **R1/R2 + A1–A5** → the **inner dev loop for non-host targets**: push-a-dylib +
  remote-trigger-reload over an async laptop↔device channel, where the only
  alternative is a full rebuild+reinstall.
...
Single ordered list; deps satisfied at every step. **Async-first**: the async story
needs no JIT spine (syscalls use the existing trampoline FFI; comptime async =
blocking `Io`), so the FFI/JIT cluster comes *after*. The compiler prerequisites are
explicit steps, not buried.

**Foundations — the compiler primitives the async story needs:**
...
2. **`declare`/`define` (construction) + `type_info`/`field_type` (reflection)** —
   comptime metaprogramming floor. Gates `race` synthesis **and** the channel
   `RecvResult`/`TryResult` types, which are sx type-fns over `declare`/`define`
   rather than an `enum($T)` language feature — type-fns (a user `($T)->Type` in
   type position) already work, e.g.
   [`Make`](../examples/0208-generics-value-param-type-function.sx) and
   [`Complex`](../examples/0201-generics-generic-struct.sx). It reuses existing
   machinery throughout. Contracts:
   ...
   - **Type-minting precedents (7):** monomorphization, protocol vtables, tuples,
     ...
     construct `TypeInfo` programmatically + `intern()`. What remains is plumbing:
     naming minted results by the instantiation's mangled name, plus input
     validation.
3. **`abi(.naked)` emission.** The `ABI` enum carries `.naked` (ast.zig, "naked, no
   prologue/epilogue"), but the label is inert: `type_resolver.zig` maps it to the
   `.default` CC and emit_llvm emits no naked attribute. The work is making `.naked`
   emit LLVM `naked` and skip prologue/epilogue lowering. Gates A2.
4. **Per-fiber `context` root + push-stack storage.** `context` is an implicit
   `*Context` parameter (comptime_vm.zig, lower.zig "Implicit Context parameter
   machinery"), not raw TLS, so it already rides the fiber stack. The unsized
   obligation is (a) where a freshly-spawned fiber's *root* `Context` comes from and
   (b) where `push Context` frames live (caller stack ⇒ fiber-local for free; a global
   root ⇒ must become per-fiber), plus the per-fiber stack-limit. A prerequisite of
   A2, not a successor.

**Async runtime — sx lib over the primitives:**
5. **A1 — `Io` interface + `context.io` + `Future` + `cancel()` API.**
6. **A2 — fiber runtime** (naked context-switch asm, bootstrap, `mmap` stacks).
7. **A3 — blocking `Io` → deterministic-sim `Io` (keystone, calibrated) → event-loop `Io`.**
8. **A5·M:1 — single-thread scheduler.**
9. **N3 — fiber-aware sync** (channels/mutex/waitgroup; `recv → RecvResult`).
10. **A6 — Cancellation.** `.canceled` in the `!` channel; per-fiber atomic
    flag (N1); every `io.*` a cancellation point; structured cancel-and-join; **masked
    during cleanup**.
11. **A4 — stdlib I/O rework** (fs/socket/process onto `context.io`).
12. **A5·N×(M:1)** — first parallel (errno-capture + `context`-fiber-local discipline).
13. **A5·M:N** — work-stealing (steal queues + migration + pinning).

**Then comptime / FFI / JIT cluster:**
14. **S1 — persistent JIT spine** → 15. **C1 — real FFI (LLVM = ABI authority, on S1)**
    → 16. **C2 — `#compiler`→`extern`** → 17. **C3 — comptime asm** (S1 + C1; +S2 if
    TLS/ctors).

**Tail:**
18. **S2 — ORC C++ shim** (highest-risk — see §8; macOS `MachOPlatform`, with no
    ELF/COFF counterpart) → 19. **R1 — dylib reload** (over `export`) → 20. **R2 —
    JIT-resident reload** (S1 + S2; **↔ async live-fiber coupling**, §8) → 21. **R3 —
...
1. **State migration across layout change** (R1/R2) → detect and rebuild/restart
   rather than migrate.
...
### 8.1 Highest technical risks, ranked
...
1. **A2 context-switch correctness** (in the async critical path). Silent stack
   ...
   *scheduling*, not the *switch*); a one-register slip is invisible until it crashes
   ...
   it silently later. → needs a dedicated **switch-stress test** (§9).
2. **`define` → tagged-union → match-codegen** (gates `race` + channels). Bounded by
   the contracts in §7 step 2: every enum stage is type-table-driven with zero AST
   coupling, identity rides existing type-fn mangled-name memoization, and
   forward-declaration for self-reference exists. What remains is plumbing — naming
   minted results by mangled name plus input validation.
3. **Deterministic-`Io` is the test keystone yet itself uncalibrated** — a buggy
   ...
   → calibrate against the blocking `Io` / property-test fixed order (§9).
...
5. **S2 ORC shim** — the only C++ in the tree, and `_Thread_local` under the bare
   LLJIT aborts. `MachOPlatform` is macOS-specific, so Linux/Windows JIT-resident
   reload and non-Mac TLS/ctor JIT are uncovered. One "M" box hides a per-OS effort.
6. **C1 args-buffer layout-vs-ABI** — "LLVM emits the call" covers the *call*, not the
   ...
   silent comptime corruption. → adversarial layout cases (§9).

---

## 9. Testing & gates

The project cadence applies: `zig build && zig build test` after every step;
xfail-then-green or behavior-lock, so no commit both adds a test AND makes it pass;
no snapshot regeneration while red; corpus = `examples/` + `issues/` with
`.exit`/`.stdout`/`.stderr`/`.ir` snapshots. This section is the design-level
verification strategy each piece must implement.

### 9.1 The async test harness = the deterministic-simulation `Io` (the keystone)
...
### 9.2 What is NOT snapshot-testable
...
coverage bound is stated loudly (a `log()`-style note in the harness), never silently
skipped.

### 9.3 Arch-sensitive lowering — atomics + context-switch
...
### 9.5 Per-piece gates (design level)
...
### 9.6 Cadence example (atomics, N1)

1. **xfail** — add `examples/17xx-atomics-fetch-add.sx` using `Atomic(i64).fetch_add`; seed the `.exit` marker → **red**, since the codegen is missing. One commit, test only.
2. **green** — emit LLVM `atomicrmw add` + ordering; the example passes; capture `.stdout` + x86_64/aarch64 `.ir` snapshots and review the diff. One commit, no new test.

Every other piece follows the same xfail→green (or behavior-lock→extend) shape.

### 9.7 Correctness gates for the silent-corruption risks

The §9.5 gates prove things *run*; the §8.1 risks are silent-corruption modes a
...
- **N1 ordering *semantics* are out of snapshot scope — state it loudly.** `.ir`
  ...
  §9.2's race carve-out; lock-free structures need the stress harness.
- **C1 args-buffer — adversarial layout cases.** Over-aligned structs, empty structs,
  ...
  "structs-by-value."
- **S2 — a TLS + C-constructor JIT test per host OS**, covering the `_Thread_local`
  abort case.
- **Hot-reload — a state-survival test** plus the live-suspended-fiber-into-stale-module
  hazard (R1/R2).
